...
async function addDocumentImpl(
    ...
    metadata: {
        ...
        batch?: boolean,
        /**
         * Where this document came from, for documents that belong to no
         * repository: 'leanix' today, whatever external datasource comes next
         * after that. Together with `sourceRef` it replaces (repo_id, path) as
         * the document's identity — see the documents.source migration in db.ts
         * for why an external source must NOT borrow a repo id.
         */
        source?: string,
        /** Stable id of this document within `source` (a LeanIX factsheet UUID). */
        sourceRef?: string,
        /**
         * The content was generated by this app from structured data and is
         * already clean markdown. Skips the LLM cleaning and summarization
         * passes; see where it is read below.
         */
        preformatted?: boolean,
        /** Summary to store when `preformatted` suppresses the generated one. */
        summary?: string
    } = {}
) {
    ...
    // ── Document identity ─────────────────────────────────────────────────────
    // Which existing row, if any, does this ingestion replace? Git documents are
    // keyed by (repo_id, path); documents from an external datasource by
    // (source, source_ref), both backed by partial unique indexes. A document
    // with neither — a manual upload — has no identity and is always inserted
    // fresh, which is the pre-existing behaviour.
    const identity: { where: string; params: unknown[] } | null =
        metadata.source && metadata.sourceRef
            ? { where: 'source = ? AND source_ref = ?', params: [metadata.source, metadata.sourceRef] }
            : metadata.repoId && metadata.path
              ? { where: 'repo_id = ? AND path = ?', params: [metadata.repoId, metadata.path] }
              : null;
    ...
    if (identity) {
        const unchanged = db.prepare(`
            ...
            WHERE ${identity.where} AND content_hash = ?
              AND EXISTS (SELECT 1 FROM chunks c WHERE c.doc_id = documents.id)
        `).get(...identity.params, contentHash) as { id: number; content: string } | undefined;
    ...
    //
    // Both passes are skipped for a `preformatted` document. That is not only a
    // saving: the cleaner's brief is to strip boilerplate and restructure prose,
    // and a 300-character factsheet rendered as a labelled field list (see
    // leanix-format.ts) has nothing to strip except the field list that is its
    // entire content. Generating a summary for it is the same trade — the caller
    // knows what the document says, because the caller wrote it.
    const cleanResult = metadata.preformatted ? null : await cleanDocument(content);
    const cleanedContent = cleanResult ? cleanResult.text : content;
    const summary = cleanResult
        ? await summarizeDocument(cleanedContent, filename)
        : (metadata.summary ?? content.slice(0, 500));
    if (cleanResult) {
        ...
        console.log(`[DocPreprocess] "${filename}": preformatted (${content.length} chars) — skipped cleaning and summarization.`);
    }
    ...
    const { docId, chunkRecords } = db.transaction(() => {
        ...
        if (identity) {
            ...
            const previous = db.prepare(`SELECT id FROM documents WHERE ${identity.where}`)
                .get(...identity.params) as { id: number } | undefined;
            if (previous) reattributeSharedClaims(previous.id);

            db.prepare(`DELETE FROM documents WHERE ${identity.where}`).run(...identity.params);
        }
        ...
        const result = db.prepare('INSERT INTO documents (filename, content, context, summary, repo_id, path, content_hash, source, source_ref) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
            filename,
            cleanedContent,
            `doc_${Date.now()}`,
            summary,
            ...
            metadata.path || null,
            contentHash,
            metadata.source ?? (metadata.repoId ? 'git' : 'upload'),
    ...
    // Nothing to audit when nothing was cleaned — a preformatted document has no
    // removals, and logging a fabricated 100%-preserved verdict would make the
    // clean log read as if the cleaner had run and approved it.
    if (cleanResult) {
        recordCleanLog({
            docId,
            filename,
